...
filter __netbird_escapeStringWithSpecialChars {
    $_ -replace '\s|#|@|\$|;|,|''|\{|\}|\(|\)|"|`|\||<|>|&','`$&'
}

[scriptblock]${__netbirdCompleterBlock} = {
    param(
            $WordToComplete,
            $CommandAst,
            $CursorPosition
    ...
    __netbird_debug "WordToComplete: $WordToComplete Command: $Command CursorPosition: $CursorPosition"
    ...
    __netbird_debug "Truncated command: $Command"
    ...
    __netbird_debug "RequestComp: $RequestComp"
    ...
    __netbird_debug "New WordToComplete: $WordToComplete"


    # Check for flag with equal sign
    $IsEqualFlag = ($WordToComplete -Like "--*=*" )
    if ( $IsEqualFlag ) {
        __netbird_debug "Completing equal sign flag"
    ...
    __netbird_debug "The completion directive is: $Directive"

    # remove directive (last element) from out
    $Out = $Out | Where-Object { $_ -ne $Out[-1] }
    __netbird_debug "The completions are: $Out"

    if (($Directive -band $ShellCompDirectiveError) -ne 0 ) {
        # Error code.  No completion.
        __netbird_debug "Received error from custom completion go code"
        return
    }
    ...
    [Array]$Values = $Out | ForEach-Object {
        ...
        __netbird_debug "Name: $Name Description: $Description"
    ...
    if (($Directive -band $ShellCompDirectiveNoSpace) -ne 0 ) {
        ...
        __netbird_debug "ShellCompDirectiveNoSpace is called"
        $Space = ""
    }

    if ((($Directive -band $ShellCompDirectiveFilterFileExt) -ne 0 ) -or
       (($Directive -band $ShellCompDirectiveFilterDirs) -ne 0 ))  {
        __netbird_debug "ShellCompDirectiveFilterFileExt ShellCompDirectiveFilterDirs are not supported"
    ...
    $Values = $Values | Where-Object {
        ...
        if ( $IsEqualFlag ) {
            __netbird_debug "Join the equal sign flag back to the completion value"
            $_.Name = $Flag + "=" + $_.Name
        }
    }
    ...
    if (($Directive -band $ShellCompDirectiveNoFileComp) -ne 0 ) {
        __netbird_debug "ShellCompDirectiveNoFileComp is called"
        ...
        switch ($Mode) {
            ...
            "Complete" {
                ...
                if ($Values.Length -eq 1) {
                    __netbird_debug "Only one completion left"

                    # insert space after value
                    $CompletionText = $($comp.Name | __netbird_escapeStringWithSpecialChars) + $Space
            ...
            Default {
                ...
                $CompletionText = $($comp.Name | __netbird_escapeStringWithSpecialChars)
                if ($ExecutionContext.SessionState.LanguageMode -eq "FullLanguage"){
                    [System.Management.Automation.CompletionResult]::new($CompletionText, "$($comp.Name)", 'ParameterValue', "$($comp.Description)")
                } else {
                    $CompletionText
...
Register-ArgumentCompleter -CommandName 'netbird' -ScriptBlock ${__netbirdCompleterBlock}